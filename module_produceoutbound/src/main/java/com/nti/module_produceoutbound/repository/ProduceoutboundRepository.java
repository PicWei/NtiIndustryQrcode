package com.nti.module_produceoutbound.repository;
...
import com.nti.module_produceoutbound.service.IProduceoutboundService;
...
public class ProduceoutboundRepository {
    ...
    public MutableLiveData<DataResult<List<ProduceoutboundOrderInfo>>> PDA_H(Paramer paramer){
        ...
        HttpUtils.getInstance().with(IProduceoutboundService.class, true).PDA_H(paramer)
                ...
                .subscribe(new Observer<JsonObject>() {
                    ...
                    public void onNext(@NotNull JsonObject jsonObject) {
                        ...
                        List<ProduceoutboundDetail> salesFactoryDetails = new ArrayList<>();
                        try {
                            for (int i = 0; i < jsonArray1.size(); i++){
                                ...
                                try {
                                    List<ProduceoutboundBarcode> barcodes = LitePal.where("UUID = ? and pcigcode = ? and isSubmit = 0", uuid, pickCode).find(ProduceoutboundBarcode.class);
                                    ...
                                    ProduceoutboundDetail salesFactoryDetail = new ProduceoutboundDetail(pickName, billPNum, billBNum, uuid, pickCode, scanBNum, String.valueOf(mscanQty));
                        ...
                        try {
                            LitePal.deleteAll(ProduceoutboundDetail.class);
                        ...
                        try {
                            for (int i = 0; i < jsonArray.size(); i++){
                                ...
                                String A_NO = null;
                                if (object.has("A_NO")){
                                    A_NO = object.get("A_NO").toString().replace("\"", "");
                                }
                                try {
                                    List<ProduceoutboundDetail> details = LitePal.where("BD_BB_UUID = ?", BB_UUID).find(ProduceoutboundDetail.class);
                                    ...
                                    ProduceoutboundOrderInfo orderInfo = new ProduceoutboundOrderInfo(BB_UUID, BB_CONTRACT_NO, BB_BT_CODE, BB_TICKET_NO, BB_WS_CODE,
    ...
    public MutableLiveData<JsonObject> updataSellListStatues(UpParamer paramer){
        ...
        HttpUtils.getInstance().with(IProduceoutboundService.class, true).updataSellListStatues(paramer)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Observer<JsonObject>() {
                    @Override
    ...
    public MutableLiveData<JsonObject> errorSignReceive(ErrorSignReceiveParamer paramer){
        final MutableLiveData<JsonObject> data = new MutableLiveData<>();
        HttpUtils.getInstance().with(IProduceoutboundService.class, true).errorSignReceive(paramer)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Observer<JsonObject>() {
                    @Override
