...
import com.nti.module_moveinbound.bean.MoveinboundBarcode;
...
public class MoveinboundDetailActivity extends BaseActivity implements View.OnClickListener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        List<MoveinboundOrderInfo> infos = LitePal.where("BB_UUID = ?", uuid).find(MoveinboundOrderInfo.class);
                    ...
                    if (result.length() != 32){
                        ...
                        for (int i = 0; i < detailList2.size(); i++){
                            ...
                            if (pcigcodesub.equals(code)){
                                ...
                                String SYSTEM_SERV = "INDUT_MOVE_STORAGE";
                                ...
                                viewModel3.sellBarcodeRecive(paramer).observe(this, new Observer<DataResult<JsonObject>>() {
                                    ...
                                    public void onChanged(DataResult<JsonObject> dataResult) {
                                        ...
                                        if (errcode == 0){
                                            ...
                                            if (code.equals("0")){
                                                MoveinboundBarcode moveinboundBarcode = new MoveinboundBarcode(uuid, pcigCode, picgname, result, scantime,scancode);
                                                moveinboundBarcode.setSubmit(true);
                                                moveinboundBarcode.saveOrUpdate("barcode = ?", result);
                                                Toast.makeText(MoveinboundDetailActivity.this, message, Toast.LENGTH_SHORT).show();
                                            }else {
                                                Toast.makeText(MoveinboundDetailActivity.this, message, Toast.LENGTH_SHORT).show();
                                            }
                                        }else if (errcode == -1){
