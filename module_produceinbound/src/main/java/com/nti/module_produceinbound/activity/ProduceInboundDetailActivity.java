...
public class ProduceInboundDetailActivity extends BaseActivity implements View.OnClickListener {
    ...
    private ProduceInboundViewModel ProduceInboundViewModel;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        ProduceInboundViewModel = new ViewModelProvider(this).get(ProduceInboundViewModel.class);
    ...
    private void loadLatelyData() {
        ProduceInboundOrderInfo info = LitePal.where("BB_UUID = ?", uuid).findFirst(ProduceInboundOrderInfo.class);
        if (info!=null){
            A_NO = info.getA_NO();
        }
    }
    ...
    protected void onStart() {
        ...
        ProduceInboundOrderInfo orderInfo = LitePal.where("BB_UUID = ?", uuid).findFirst(ProduceInboundOrderInfo.class);
    ...
    public void onClick(View view) {
        ...
        if (view.getId() == R.id.confirm_brn){
            ...
            int barcode_count = LitePal.where("UUID = ? and isSubmit = 0", uuid).count(ProduceInboundBarcode.class);
            if (barcode_count == 0){
                            ...
                            .asConfirm(null, "当前单据还未扫描完全，确认完成？",
                                    ...
                                    new OnConfirmListener() {
                                        ...
                                        public void onConfirm() {
                                            ...
                                            ProduceInboundViewModel.updataSellListStatues(upParamer).observe(ProduceInboundDetailActivity.this, new Observer<JsonObject>() {
                                                @Override
    ...
    private void handleScannerResult(String result) {
        ...
        int count = 0;
        int size = LitePal.where("barcode = ?", result).count(ProduceInboundBarcode.class);
        ...
        for (int i = 0; i < detailList2.size(); i++) {
            ...
            if (pcigcodesub.equals(code)) {
                ...
                ProduceInboundBarcode ProduceInboundBarcode = new ProduceInboundBarcode(uuid, pcigCode, pcigName, result, scantime, scancode);
                ProduceInboundBarcode.save();
                ...
                sellBarcodeReciveViewModel.sellBarcodeRecive(paramer).observe(this, new Observer<DataResult<JsonObject>>() {
                    ...
                    public void onChanged(DataResult<JsonObject> dataResult) {
                        ...
                        if (errcode == 0) {
                            ...
                            ProduceInboundBarcode barcode = new ProduceInboundBarcode(uuid, pcigCode, picgname, result, scantime, scancode);
                            barcode.setSubmit(true);
                            barcode.save();
                            Toast.makeText(ProduceInboundDetailActivity.this, "sucess", Toast.LENGTH_SHORT).show();
...
//                                }
    ...
    private void sendErrorCode(String barcode){
        ...
        ProduceInboundViewModel.errorSignReceive(esrparamer).observe(this, new Observer<JsonObject>() {
            @Override
            public void onChanged(JsonObject jsonObject) {
                if (jsonObject == null){
                    Toast.makeText(ProduceInboundDetailActivity.this, "网络异常", Toast.LENGTH_SHORT).show();
