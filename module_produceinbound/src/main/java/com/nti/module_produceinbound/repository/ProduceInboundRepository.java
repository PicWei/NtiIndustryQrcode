package com.nti.module_produceinbound.repository;
...
import com.nti.module_produceinbound.service.IProduceInboundService;
 ...
 * @describe
 */
public class ProduceInboundRepository {

    public MutableLiveData<DataResult<List<ProduceInboundOrderInfo>>> PDA_H(Paramer paramer){
        final MutableLiveData<DataResult<List<ProduceInboundOrderInfo>>> data = new MutableLiveData<>();
        final List<ProduceInboundOrderInfo> orderInfos = new ArrayList<>();
        HttpUtils.getInstance().with(IProduceInboundService.class, true).PDA_H(paramer)
                ...
                .subscribe(new Observer<JsonObject>() {
                    ...
                    public void onNext(@NotNull JsonObject jsonObject) {
                        ...
                        List<ProduceInboundDetail> salesFactoryDetails = new ArrayList<>();
                        try {
                            for (int i = 0; i < jsonArray1.size(); i++){
                                ...
                                try {
                                    List<ProduceInboundBarcode> barcodes = LitePal.where("UUID = ? and pcigcode = ? and isSubmit = 0", uuid, pickCode).find(ProduceInboundBarcode.class);
                                    ...
                                    ProduceInboundDetail salesFactoryDetail = new ProduceInboundDetail(pickName, billPNum, billBNum, uuid, pickCode, scanBNum, String.valueOf(mscanQty));
                        ...
                        try {
                            LitePal.deleteAll(ProduceInboundDetail.class);
                        ...
                        try {
                            for (int i = 0; i < jsonArray.size(); i++){
                                ...
                                try {
                                    List<ProduceInboundDetail> details = LitePal.where("BD_BB_UUID = ?", BB_UUID).find(ProduceInboundDetail.class);
                                    ...
                                    ProduceInboundOrderInfo orderInfo = new ProduceInboundOrderInfo(BB_UUID, BB_CONTRACT_NO, BB_BT_CODE, BB_TICKET_NO, BB_WS_CODE,
                        ...
                        try {
                            LitePal.deleteAll(ProduceInboundOrderInfo.class);
                            LitePal.saveAll(orderInfos);
                //            data.setValue(orderInfos);
                            DataResult<List<ProduceInboundOrderInfo>> dataResult = new DataResult<>(0, orderInfos);
                    ...
                    public void onError(@NotNull Throwable e) {
                        DataResult<List<ProduceInboundOrderInfo>> dataResult = new DataResult<>(-1, null);
                        data.setValue(dataResult);
                    }
    ...
    public MutableLiveData<JsonObject> updataSellListStatues(UpParamer paramer){
        ...
        HttpUtils.getInstance().with(IProduceInboundService.class, true).updataSellListStatues(paramer)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Observer<JsonObject>() {
                    @Override
    ...
    public MutableLiveData<JsonObject> errorSignReceive(ErrorSignReceiveParamer paramer){
        final MutableLiveData<JsonObject> data = new MutableLiveData<>();
        HttpUtils.getInstance().with(IProduceInboundService.class, true).errorSignReceive(paramer)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Observer<JsonObject>() {
                    @Override
