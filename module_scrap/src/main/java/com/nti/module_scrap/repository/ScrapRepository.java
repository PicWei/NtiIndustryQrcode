...
import com.nti.module_scrap.service.IScrapService;
        ...
        HttpUtils.getInstance().with(IScrapService.class).PDA_H(paramer)
                ...
                .subscribe(new Observer<JsonObject>() {
                    ...
                    public void onNext(@NotNull JsonObject jsonObject) {
                        ...
                        List<ScrapDetail> salesFactoryDetails = new ArrayList<>();
                        try {
                            for (int i = 0; i < jsonArray1.size(); i++){
                                ...
                                try {
                                    List<ScrapBarcode> barcodes = LitePal.where("UUID = ? and pcigcode = ? and isSubmit = 0", uuid, pickCode).find(ScrapBarcode.class);
                                    ...
                                    ScrapDetail salesFactoryDetail = new ScrapDetail(pickName, billPNum, billBNum, uuid, pickCode, scanBNum, String.valueOf(mscanQty));
                        ...
                        try {
                            LitePal.deleteAll(ScrapDetail.class);
                        ...
                        try {
                            for (int i = 0; i < jsonArray.size(); i++){
                                ...
                                try {
                                    List<ScrapDetail> details = LitePal.where("BD_BB_UUID = ?", BB_UUID).find(ScrapDetail.class);
                                    ...
                                    ScrapOrderInfo orderInfo = new ScrapOrderInfo(BB_UUID, BB_CONTRACT_NO, BB_BT_CODE, BB_TICKET_NO, BB_WS_CODE,
                        ...
                        try {
                            LitePal.deleteAll(ScrapOrderInfo.class);
                            LitePal.saveAll(orderInfos);
                            DataResult<List<ScrapOrderInfo>> dataResult = new DataResult<>(0, orderInfos);
                    ...
                    public void onError(@NotNull Throwable e) {
                        DataResult<List<ScrapOrderInfo>> dataResult = new DataResult<>(-1, null);
                        data.setValue(dataResult);
                    }
