...
public class ScrapDetailActivity extends BaseActivity implements View.OnClickListener{
    ...
    private void loadLatelyData() {
        ScrapOrderInfo info = LitePal.where("BB_UUID = ?", uuid).findFirst(ScrapOrderInfo.class);
        if (info!=null){
    ...
    protected void onStart() {
        ...
        ScrapOrderInfo orderInfo = LitePal.where("BB_UUID = ?", uuid).findFirst(ScrapOrderInfo.class);
    ...
    public void onClick(View view) {
        ...
        if (view.getId() == R.id.confirm_brn){
            ...
            int barcode_count = LitePal.where("UUID = ? and isSubmit = 0", uuid).count(ScrapBarcode.class);
    ...
    private void handleScannerResult(String result) {
        ...
        int count = 0;
        int size = LitePal.where("barcode = ?", result).count(ScrapBarcode.class);
        ...
        for (int i = 0; i < detailList2.size(); i++) {
            ...
            if (pcigcodesub.equals(code)) {
                ...
                ScrapBarcode ScrapBarcode = new ScrapBarcode(uuid, pcigCode, pcigName, result, scantime, scancode);
                ScrapBarcode.save();
                ...
                sellBarcodeReciveViewModel.sellBarcodeRecive(paramer).observe(this, new Observer<DataResult<JsonObject>>() {
                    ...
                    public void onChanged(DataResult<JsonObject> dataResult) {
                        ...
                        if (errcode == 0) {
                            ...
                            ScrapBarcode barcode = new ScrapBarcode(uuid, pcigCode, picgname, result, scantime, scancode);
                            barcode.setSubmit(true);
                            barcode.save();
                            Toast.makeText(ScrapDetailActivity.this, "sucess", Toast.LENGTH_SHORT).show();
