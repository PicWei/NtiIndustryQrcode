...
public class ReturnInboundDetailActivity extends BaseActivity implements View.OnClickListener {
    ...
    private ReturnInboundViewModel ReturnInboundViewModel;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        ReturnInboundViewModel = new ViewModelProvider(this).get(ReturnInboundViewModel.class);
        ...
        if (view.getId() == R.id.confirm_brn){
            ...
            if (barcode_count == 0){
                ...
                if (!pum.equals(scanednum)){
                            ...
                            .asConfirm(null, "当前单据还未扫描完全，确认完成？",
                                    ...
                                    new OnConfirmListener() {
                                        ...
                                        public void onConfirm() {
                                            ...
                                            ReturnInboundViewModel.updataSellListStatues(upParamer).observe(ReturnInboundDetailActivity.this, new Observer<JsonObject>() {
                                                @Override
                    ...
                    com.nti.lib_common.bean.UpdataStatuesParamer updataStatuesParamer = new UpdataStatuesParamer(uuid, BB_STATE, SYSTEM_SERVICE_TYPE);
                    com.nti.lib_common.bean.UpParamer upParamer = new UpParamer(updataStatuesParamer);
                    ReturnInboundViewModel.updataSellListStatues(upParamer).observe(ReturnInboundDetailActivity.this, new Observer<JsonObject>() {
                        @Override
                        public void onChanged(JsonObject jsonObject) {
                            if (jsonObject == null){
...
//                                }
    ...
    private void sendErrorCode(String barcode){
        SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        String BI_FEEDBACK_TIME = format.format(new Date());
        com.nti.lib_common.bean.ErrorBarcode errorBarcode = new com.nti.lib_common.bean.ErrorBarcode(barcode, BI_SCANNER_CODE, BI_FEEDBACK_TIME);
        List<ErrorBarcode> errorBarcodes = new ArrayList<>();
        errorBarcodes.add(errorBarcode);
        com.nti.lib_common.bean.ErrorBarcodeParamer errorBarcodeParamer = new ErrorBarcodeParamer(uuid, SYSTEM_SERVICE_TYPE, errorBarcodes);
        com.nti.lib_common.bean.ErrorSignReceiveParamer esrparamer = new ErrorSignReceiveParamer(errorBarcodeParamer);

        ReturnInboundViewModel.errorSignReceive(esrparamer).observe(this, new Observer<JsonObject>() {
            @Override
            public void onChanged(JsonObject jsonObject) {
                if (jsonObject == null){
                    Toast.makeText(ReturnInboundDetailActivity.this, "网络异常", Toast.LENGTH_SHORT).show();
